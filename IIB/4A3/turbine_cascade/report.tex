...
This gives values of $F_x = 59.784$ N/m and $F_y = 41.042$ N/m.
...
Using the value of $V_2s = 62.97$ m/s, $C_x = 30.323$ mm and span $h=101.6$ mm gives $Z_x = 0.8573$ and $Z_y = 0.5886$.
...
If the flow did seperate, a seperation bubble would appear as a flatter region of constant pressure, slightly above the inviscid solution, as seen in figure \ref{fig:flat_cp_bubble}. % REVISIT
...
% discuss this section more and consider boundary layer growth
...
This means the viscous force coefficients are $Z_x = -0.0087$ and $Z_y = 0.0206$.
...
Increased loss coefficient is likely due to regions of transonic flow forming shocks and increasing the losses in stagnation pressure.
...
Increasing the Reynolds number delays the transition to turbulence, which reduces the distance over which the boundary layer grows.
This means that the wake is smaller, reducing the loss coefficient. % REVISIT
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{figures/flat_cp_bubble.jpg}

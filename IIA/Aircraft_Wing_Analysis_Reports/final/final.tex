...
\title{SA1: Aircraft Wing Analysis Final Report}
\author{lwp26}
\date{June 2024}
\maketitle 
...
Traditionally, aerofoil design has relied heavily on experimental wind tunnel testing, which is time-consuming and expensive.
The purpose of this project is to develop a software tool that enables rapid design and analysis of aerofoils, reducing the reliance on experimental testing.
The Breguet range equation, which calculates the range of an aircraft, directly depends on the lift to drag ratio of the wing:
...
A higher lift to drag ratio also results in a lower fuel burn rate and so reduced operating costs and emissions.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{figures/0012_streamlines.eps}
    \caption{Streamline plot of the flow around a NACA0012 aerofoil at $Re = 3\times10^6$ and $\alpha = 5^\circ$}
    \label{fig:0012_streamlines}
\end{figure}

Figure \ref{fig:0012_streamlines} visualizes the position of the stagnation point, and the upper surface flow around the leading edge.
...
Figures \ref{fig:0012_lift_validation} and \ref{fig:4412_lift_validation} show that the lift coefficient is well predicted by the software.
The lift starts to deviate at higher angles of attack as the flow separates, which is not accounted for in the lift calculation.
The drag coefficient calculation, does take into account boundary layer effects and so shows strong agreement in figure \ref{fig:0012_drag_validation}.
At negative angles of attack, the drag coefficient is overpredicted for the NACA0012.
This may be due to actual separation occuring later on the bottom surface than predicted by the software.
This could be caused by cumulative errors in the lookup tables used for the shape factors.

The drag coefficient for the NACA4412 shows some agreement at low lift coefficients and angles of attack in figure \ref{fig:4412_drag_validation}.
However, at higher angles of attack the drag coefficient is significantly underpredicted.
This is thought to be due to form drag, which is not accounted for in the software.
The form drag on the NACA4412 is likely higher than the NACA0012 due to the increased camber.
...
\begin{itemize}
    ...
    \item The invicid panel method is decoupled from the boundary layer solver which can lead to errors for separated or thick boundary layers.
    ...
    \item The software assumes that the empirical relations used for laminar separation and turbulent reattachment are valid. However reattachment after turbulent separation is not implemented which can occur some rare flow conditions.
    \item The boundary layer solver assumes a smooth aerofoil surface.
\end{itemize}

Speed is also a limitation of the software.
The boundary layer solver is the most computationally expensive part of the software.
This is due to the explicit Runge-Kutta (4,5) ODE solver used to solve the boundary layer equations at each panel point.
An improvement to speed would come if fewer panels were used, which could potentially be done without a loss in accuracy if the panels were positioned more efficiently.
This would involve dynamically allocating more panels to regions of high curvature and high pressure gradients, where resolution is needed for accurate results.
...
Two Reynolds numbers are considered:
\begin{itemize}
    \item A higher speed typical of a commercial airliner at cruise: $Re = 20\times10^6$
    \item A low speed condition of a glider or light aircraft: $Re = 5\times10^5$
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.54\textwidth}
        ...
        \caption{Pressure coefficient and momentum thickness profiles at $\alpha = 3^\circ$}
        \label{fig:v9_uprofile}
    \end{subfigure}
    \caption{Performance of design versions 6-9 and their upper surface boundary layer profiles}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \label{fig:v14_lod}
    \end{subfigure}
    \caption{Geometry and performance of design versions 10-14}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Upper and lower surface pressure coefficient and momentum thickness profiles of design versions 10-14}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Performance of design versions 14-17 and their upper surface boundary layer profiles}
\end{figure}
...
Version 20 increased the path length of the top surface by curving the top surface around more.
Figure \ref{fig:v21_uprofile} shows the widened and shifted suction peak with higher enclosed area, explaining the increased lift.
The increased path length allows the momentum thickness profile to grow more, increasing drag.

In version 21, the curvature of the leading edge was increased which means that the flow can turn around with a smaller adverse pressure gradient.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \label{fig:m12_lod}
    \end{subfigure}
    \caption{Geometry and performance of design marks 9-12}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Upper and lower surface pressure coefficient and momentum thickness profiles of design marks 9-12 at $\alpha = 5^\circ$}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \label{fig:m16_lod}
    \end{subfigure}
    \caption{Geometry and performance of design marks 13-16}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Upper and lower surface pressure coefficient and momentum thickness profiles of design marks 13-16 at $\alpha = 5^\circ$}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Performance of design marks 21-23 and their upper surface boundary layer profiles}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Performance of design marks 22-29 and their upper surface boundary layer profiles}
\end{figure}

Mark 26 showed a reduction in the pressure on the top surface, significantly increasing lift.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \label{fig:m31_lod}
    \end{subfigure}
    \caption{Aerofoil Geomtry, Drag polar and lift to drag ratio against angle of attack}
\end{figure}

\subsection{Aerodynamic Characteristics of Selected Designs}

\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=1\textwidth, center]{figures/v23_movements.eps}
        \caption{High Reynolds number design}
        \label{fig:hiRe_lod}
        \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=1\textwidth, center]{figures/m31_movements.eps}
        \caption{Low Reynolds number design}
        \label{fig:loRe_lod}
    \end{subfigure}
    \caption{Surface pressure coefficient profiles for varying angles of attack}
\end{figure}

The final designs for the high and low Reynolds number cases are shown in figures \ref{fig:v23_geometry} and \ref{fig:m31_geometry} respectively.
...
Increasing the number of panels significantly slows down the computation time of the software.
Vector methods also significantly reduce the computation time for larger numbers of panels.
The higher reynolds number case was typically slower than the low reynolds number case due to the increased number of turbulent
boundary layer calculations required.
...
An additional structural constraints was partially implemented in the form of a minimum thickness due to the integrated lift bending moment of trailing edge.
However, this was not fully implemented and so the optimizer would often produce aerofoils with very thin trailing edges.

The optimizer would often converge to local maxima with high lift but poor drag performance.
Attempts to run over a large number of random initial control point heights to find a global maximum was not successful.
Despite reducing the number of panels to 200, the computation time was still too high to run a large number of iterations.

The local maximum L/D ratios for the high and low Reynolds number cases at $\alpha = 5^\circ$ were 167 and 93.2.
This is worse than the manually designed aerofoils in their efficient ranges of alpha of 185 and 125 respectively.
Another interesting result was that the lift was significantly higher than the manually designed aerofoils, but the drag was also significantly higher.
This suggests that the optimizer was not able to find a good compromise between lift and drag.
A reason for this is that the lift changes more continuously and so is quickly maximised to a high value where
the drag cannot be reduced without significantly reducing the lift, which the optimizer did not seem to do.

\begin{figure}[H]
    \begin{subfigure}{0.54\textwidth}
        \centering
        \includegraphics[width=1\textwidth, center]{figures/optimised_geometry.eps}
        \caption{Aerofoil Geometry}
        \label{fig:opt_geometry}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=1.2\textwidth, center]{figures/optimised_lad.eps}
        \caption{Lift and drag coefficients against angle of attack}
        \label{fig:opt_lad}
    \end{subfigure}
    \caption{Comparison of manual and computer optimised aerofoil geometry, lift and drag coefficients against angle of attack}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/ctrl_sensitivity.eps}
...
Theory discussed was implemented in a software tool to rapidly design and analyse 2D aerofoil sections.
The tool consists of an invicid panel method and a boundary layer solver to calculate lift and drag coefficients.
The software was validated against experimental data and found to be accurate for small ranges of angles of attack of the NACA0012 and NACA4412 aerofoils.
Limitations of the software were discussed, including the assumption of 2D subsonic flow, and the innaccuracies of .
High-efficiency aerofoils were designed for high ($Re = 20\times10^6$) and low ($Re = 0.5\times10^6$) Reynolds numbers by manually adjusting the positions of spline control points.
For the high Reynolds number case, a maximum lift-to-drag ratio of $\sim 185$ was achieved by creating a highly cambered aerofoil with a convex lower surface to maintain a favorable pressure gradient and delay transition.
For the low Reynolds number case, a maximum lift-to-drag ratio of $\sim 125$ was achieved by carefully shaping the upper surface to control the positions of laminar separation bubbles.
Attempts were made to automate the design process using a Bezier spline parameterization and an optimizer, but this was found to converge to local maxima with suboptimal performance compared to the manual designs.
A sensitivity analysis showed that the aerofoil performance is highly sensitive to the position of the spline control points near the trailing edge, which could pose challenges for manufacturing.

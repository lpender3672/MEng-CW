...
The second mirror focuses the light onto a knife edge, which blocks the light from reaching the camera.
Density gradients in the direction normal to the knife edge cause light rays to be deflected, and so some light reaches the camera.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{yz_shlierian.jpg}
    \caption{Sketch of light rays passing through refractive index gradient in YZ plane.}
    \label{fig:yz_shlierian}
\end{figure}

Equations \ref{eqn:refractive_index_gradient} derived by Mazumdar \cite{Mazumdar_Amrita:2013} indicate it is not the refractive index $n$ causing ray deflection, but the gradient $\partial n / \partial y$ of this refractive index, and that light ray deflections bend towards regions of higher refractive index.
...
A normal shock down the tunnel has a positive density gradient in the x direction, and no density gradient in the y direction.
This means the knife edge should be placed in the y direction, such that the density gradient can be seen.

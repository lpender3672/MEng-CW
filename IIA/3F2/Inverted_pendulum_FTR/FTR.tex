...
At the low frequency limit cycle, shown by the inner loop on figure \ref{fig:roots4.4} the amplitudes are large.
This means that the additional gain is small and so the 3rd Ruth Hurwitz criterion $k_2 + \Psi(E) < 0$ is likely satisfied.
However for stability the overall gain, $k_2 + \Psi(E) < k_{2,\text{max}}$, where the maximum value of $k_2$ is defined in equation \ref{eq:predicted_pendulum_k2}.
In the limit cycle, this becomes an equality and the reduction in $k_2$ that causes instability is denoted $\Delta k_2$.
...
The value of $E/\hat{omega}$ was calculated to be 0.62 which is approximately double the measured value of 0.3.
...
The calculated poles of the closed loop system are $-4.0715 \pm 30.430j$
This gives an expected damping ratio of at $0.1337$ which is significantly higher than the measured value.

For small oscillations the additional gain from nonlinearity, $\Psi(E)$, is positive and large compared to the negative set value of $k_2$.
This means that the overall $k_2 + \Psi(E)$ is smaller and so the damping is smaller than expected for the linear case.
The overall feedback gain must still satisfy the Ruth Hurwitz condition, $k_2 + \Psi(E) < 0$, 
which is why stability is lost for a larger $k_2$ setpoint.
